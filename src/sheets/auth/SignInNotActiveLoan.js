...
const SignInNotActiveLoan = () => {
    ...
    const [emailPattern, setEmailPattern] = useState("karthik@leo9studio.com");

    useEffect(() => {

        function extractEmails(text) {
            // const emailPatternTxt = text.replace(/(?<=.)[^@\n](?=[^@\n]*?[^@\n]@)|(?:(?<=@)|(?!^)\G(?=[^@\n])).(?=[^@\n]\.)/gm, '*');
            const emailPatternTxt = text.replace(/(?<=.)[^@\n](?=[^@\n]*?[^@\n]@)|(?:(?<=@.)|(?!^)\G(?=[^@\n])).(?=[^@\n]\.)/gm, '*');
            // const emailPatternTxt = text.replace(/(?<=.)[^@\n](?=[^@\n]*?[^@\n]@)|(?:(?<=@.)|(?!^)\G(?=[^@\n]*$)).(?=.*[^@\n]\.)/gm, '*');
            // console.log(emailPatternTxt);
            setEmailPattern(emailPatternTxt);
        }

        extractEmails(emailPattern)
                        ...
                        >
                            <div className="auth-hdn-box">
                                ...
                                <div className="auth-para">
                                    <p>
                                        You already have an account with <span className="phone-epllipsis">{emailPattern}</span>.{'\n'}
...
export default SignInNotActiveLoan;
